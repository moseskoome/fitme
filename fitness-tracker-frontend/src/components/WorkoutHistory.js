...
const WorkoutHistory = () => {
  ...
  const groupByExerciseAndDate = (data) => {
    const grouped = data.reduce((acc, workout) => {
      ...
      acc[workout.exercise][date].push(workout);
      return acc;
    }, {});
        ...
        <div>
          {Object.keys(groupedWorkouts).map((exercise) => (
            <div key={exercise} className="exercise-table-container">
              <h3>{exercise}</h3>
              {Object.keys(groupedWorkouts[exercise]).map((date) => (
                <div key={date}>
                  <h4>{date}</h4>
                  <table className="exercise-table">
                    ...
                    <tbody>
                      {groupedWorkouts[exercise][date].map((workout) => (
                        <tr key={workout._id}>
                          <td>
